    ...
    for i in range(0,len(alertList)):
        ...
        if(case != 0):
            ...
            if(case == 1 and items[reward['ItemType']] == "Nitain Extract"):
                await bot.send_message(channel, "@everyone :Nitain:")
            ...
            #Assemble message
            #embed=discord.Embed(title="Alert for:", description=reward, color=color)
            #embed=discord.Embed(title=reward, color=color)
            #embed.add_field(name=missionType, value=location, inline=True)
